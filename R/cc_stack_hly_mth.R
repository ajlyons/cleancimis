#' Stack hourly values for a set of stations that have been downloaded by month
#'
#' Stack hourly values for a set of stations that have been downloaded by month
#'
#' @param year Year(s) to stack, integer
#' @param month Months(s) to stack, integer
...
#'
#' @details
#' This will import the hourly values for a set of stations that have been downloaded in monthly aggregations, and append them in
#' a single CSV file for the year.
#' This presumes you've already download the monthly aggregations of CIMIS data, and unzipped the CSV files for the stations of interest.
#'
#' @importFrom readr read_csv write_csv cols col_character
#' @importFrom dplyr tibble filter pull mutate
...
cc_stack_hly_mth <- function(year,
                             month = NULL,
                             stations = NULL,
  ...
  if (FALSE %in% (year %in% 2024:2025)) stop("year must be 2024 to 2025")
  ...
  if (is.null(stations)) {
    stop("Stations NULL not yet supported")
  } else {
  ...
  month_nums_lst <- setNames(as.list(1:12), tolower(month.abb))

  files_tbl <- tibble(
    filename = files_chr,
    year = as.integer(substr(files_chr, 1, 4)),
    month_abbrev = substr(files_chr, 6, 8),
    stid = as.integer(substr(files_chr, 15, 17))
  ) |> mutate(month_int = as.integer(month_nums_lst[month_abbrev]))
  ...
  for (i in 1:length(year)) {
    ...
    files2stack_chr <- files_tbl |>
      filter(year == this_year, month_int %in% .env$month_use, stid %in% .env$stations) |>
      pull(filename)
    ...
    if (proceed_yn) {
      message(crayon::green(paste0(" - concatenating ", length(files2stack_chr),
                                   " CSVs for ", this_year)))
